...
export async function storeAdminEmailSender(emailData) {
  try {
...
export async function adminEmailSender(emailData) {
  try {
    const htmlData = `Your store '${companyName}' have received an ${emailData.paymentType} order with ${emailData.itemsCount} items and value of ₹${emailData.totalPrice} from ${emailData.customerName}.`;
